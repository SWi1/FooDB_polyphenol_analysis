---
title: "Q5_Polyphenol_Quantification_ASAMerge"
...
__Required Input Files__.                          
  ...
  - *FooDB_phenol_content.csv.bz2* - Output from Q4_Polyphenol_Quantification_Units
  - *Food_updated.csv* - 01_FooDB_FooDBCleaning.ipynb
  - *FooDB_PE_RF.csv* - Output from 03_Polyphenol_Quantification_RF.Rmd
  - *ASA_FooDB_Food_Matrix.csv* - Output from Other_Food_Matrix.Rmd
...
This script merges ingredients within recall data to their ingredient equivalent in FooDB. The resulting output is subsequently merged with the FooDB polyphenol content file. 
    1) Merger of dietary recall data with FooDB matches and then polyphenol content     - This step also includes the creation of a new column, **pp_consumed**, which is the amount of polyphenols consumed in milligrams, which incorporates all possible retention factor data. 
...
#Load Food Matrix Data
food_matrix = read.csv('data/ASA_FooDB_Food_Matrix.csv') %>%
  select(c(FoodCode, Ingredient_code, FoodMatrix))
```
...
ASA = left_join(ingredients, matched, by = 'Ingredient_code') %>%
  left_join(food_matrix, by = c('FoodCode', 'Ingredient_code')) %>%
  relocate(FoodMatrix, .after = WWEIA.Category.number) %>%
...
message('Number of unique foods in ASA: ',
  ...
  '\nNumber of unique compounds in ASA: ',
  length(unique(ASA$compound_id)))
```
...
'Nectar' Beverages, Adjust for percent juice. Only a handful of users.
...
tea = ASA_FooDB_adj %>%
  ...
  dplyr::mutate(pp_consumed = (orig_content_avg_RFadj*0.01) *
           (Ingredient_consumed_g*(1/50)))
```
...
ASA_FooDB_updated = ASA_FooDB_adj %>%
  dplyr::filter(!food_id %in% tea_food_ids) %>% #take out old values
  dplyr::filter(!food_id %in% coffee_food_ids) %>% #take out old values
  dplyr::filter(!FoodCode %in% nectar_ids) %>% #take out old values
  dplyr::mutate(pp_consumed = (orig_content_avg_RFadj*0.01) * 
                  Ingredient_consumed_g) %>%
  dplyr::full_join(tea) %>% #add new values
  dplyr:: full_join(coffee) %>% #add new values %>%
  dplyr:: full_join(nectar) %>%
  dplyr::select(-Modified) %>%
  dplyr::relocate(pp_consumed, orig_content_avg_RFadj, .before = orig_unit)
```
