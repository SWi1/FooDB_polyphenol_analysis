---
title: "Q4_Polyphenol_Quantification_Content"
...
__Required Input Files__  
  ...
  - *FooDB_PE_RF.csv* - contains FooDB/Phenol Explorer matches with retention factor data. Also includes those foods that did not match. Output from Q3_Polyphenol_Quantification.
...
message('Number of unique foods in FooDB content (unaggregated): ',
  length(unique(FooDB_phenol_content$food_id)),
  
  '\nNumber of unique compounds in FooDB content (unaggregated): ',
  length(unique(FooDB_phenol_content$compound_id)),
  
  '\nNumber of unique foods in FooDB content (quantified, unaggregated): ',
  nrow(FooDB_phenol_content %>% dplyr::filter(!is.na(orig_content)) %>% select(food_id) %>% distinct()),
  
  '\nNumber of unique compounds in FooDB content (quantified, unaggregated): ',
  nrow(FooDB_phenol_content %>% dplyr::filter(!is.na(orig_content)) %>% select(compound_id) %>% distinct()))
```
...
FooDB_phenol_content_updated = FooDB_phenol_content %>%
  dplyr::filter(!citation == 'PHENOL EXPLORER') %>% #remove Phenol Explorer Entries
  dplyr::full_join(RF) %>% #Add in our Phenol Explorer Entries that have been matched
  dplyr::mutate(orig_unit_updated = 'mg/100g') %>% 
  dplyr::group_by(compound_public_id, food_id) %>%
  dplyr::mutate(orig_content_avg = mean(orig_content, na.rm = TRUE)) %>% #Average
  dplyr::ungroup() %>% #NEW
  ...
  dplyr::distinct(food_id, compound_id, .keep_all = TRUE) %>% #Removes ind. entries
...
### 3) Polyphenol Totals and Updating the Content File
...
Unquantified_Adjusted = Food_Sums_Full_List %>% 
  ...
  dplyr::mutate(total_pp_adj = 'NA',
                total_pp_RFadj = 'NA',
                Status_adj = 'Unquantified but Predicted or Expected') %>%
  dplyr::mutate(total_pp_adj = as.numeric('NA'),
